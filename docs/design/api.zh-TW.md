...
| 參數 | 型別 | 必填 | 說明 |
| --- | --- | --- | --- |
| `status` | string | 否 | 以 `ACTIVE` 或 `RESOLVED` 篩選。省略時兩者都回傳。 |
...
**查詢參數：** `status`（選填，`ACTIVE` | `RESOLVED`）、`limit`（選填，預設 20，上限 100）。

**回應 `200`** — `{ "data": [ ...alerts ] }`。

**回應 `400`** — `status` 不屬於 `ACTIVE` | `RESOLVED` 時為 `INVALID_QUERY_PARAMETER`（同樣適用於 §4.4 的 `status` 篩選）。超出範圍的 `limit` 值會被截限（clamp），不會被拒絕。
...
| `400` | `INVALID_QUERY_PARAMETER` | 查詢參數值超出其記載的值域（例如 `status` 不在 `ACTIVE`/`RESOLVED` 內）。 |
