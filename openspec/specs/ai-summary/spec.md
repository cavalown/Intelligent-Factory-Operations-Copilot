...
## Purpose
Generate, persist, and serve LLM-backed operational summaries at machine and factory scope. The Insight Service explains data owned by the Machine/Event/Alert services (key design rule 3: AI explains data, it does not replace it) — it gathers recent context, calls a provider-agnostic LLM client synchronously (MVP: no job queue), stores results in `ai_summaries` with `inputEventIds` traceability, and keeps LLM failures isolated from dashboard availability. Introduced by change `add-insights-module` (2026-07-10).
