...
## 目的
在機台與工廠兩種範圍產生、持久化並提供以 LLM 為後盾的營運摘要。Insight Service 解釋由 Machine/Event/Alert 服務擁有的資料（關鍵設計規則 3：AI 解釋資料，不取代資料）— 它蒐集近期脈絡、同步呼叫供應商無關的 LLM client（MVP：沒有工作佇列）、把結果連同 `inputEventIds` 可追溯性存入 `ai_summaries`，並讓 LLM 失敗與儀表板可用性隔離。由變更 `add-insights-module`（2026-07-10）引入。
