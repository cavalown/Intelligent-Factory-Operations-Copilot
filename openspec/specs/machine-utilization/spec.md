# machine-utilization Specification

## Purpose
Track machine status transitions as a rebuildable projection (written best-effort by the machines projection consumer — never allowed to abort the primary projection update) and serve rolling-24h time-in-status (operating / stopped / idle) per machine and factory-wide, with the bootstrap approximation explicitly flagged. Introduced by change `dashboard-operational-metrics` (2026-07-11).
