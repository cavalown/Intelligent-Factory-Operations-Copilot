# machine-utilization 規格

## 目的
把機台狀態轉移追蹤為可重建的投影（由 machines 投影 consumer 以 best-effort 寫入 — 永遠不允許中止主投影更新），並提供每台機台與全工廠的滾動 24 小時分狀態時間（operating / stopped / idle），且明確標示 bootstrap 近似。由變更 `dashboard-operational-metrics`（2026-07-11）引入。
